...
# + Перед входом проверяем направление мw_30м(talib)
...
TREND_THRESHOLD = 1.0010 # порог силы тренда при поиске тренда
...
def TAKE_PROFIT_CALC_SHORT(TAKE_PROFIT_PERC, price):
    take_profit = price / (1 + TAKE_PROFIT_PERC/100)
    return take_profit

def STOP_LOSS_CALC_SHORT(STOP_LOSS, price):
    stop_loss = round(price * (1 + (STOP_LOSS/100)), 5)
    return stop_loss 
...
def bot(search_trend_power=False, open_position=False, search_pattern = False, open_position_3_4 = False, open_position_1_2 = False, open_position_1_4 =False):
    ...
    while True:
        ...
        while search_trend_power == True:   
            ...
            if  df_5m['trend_power'].iloc[-1] > TREND_THRESHOLD:  # если сила тренда больше порога
                ...
                search_pattern = True # включение поиска паттерна ПЕРЕГИБ НА ПАДЕНИЕ 5_М
                ...
                while search_pattern == True:   
                    ...
                    ########################################################################################################
                    # если паттерн нашли 
                    if (df_5m['close'].iloc[-1] < df_5m['sma'].iloc[-1]) and (df_5m['trend_power'].iloc[-1] < 1.0):
                        time.sleep(4)
                        ####################################################################################
                        ...
                        if  df_30m['trend_power'].iloc[-1] < TREND_THRESHOLD_30m:
                            ...
                            price_sma = df_5m['sma'].iloc[-1]
                            qty_current = POS_SIZE
                            TP_1 = TAKE_PROFIT_CALC_SHORT(TP_1_PROC, price_entry)  # рассчитываем тейки  
                            TP_2 = TAKE_PROFIT_CALC_SHORT(TP_2_PROC, price_entry)    
                            TP_3 = TAKE_PROFIT_CALC_SHORT(TP_3_PROC, price_entry)   
                            TP_4 = TAKE_PROFIT_CALC_SHORT(TP_4_PROC, price_entry) 
                            stop_loss = STOP_LOSS_CALC_SHORT(STOP_LOSS_PROC, price_sma) 
                            # открываем позицию
                            send_limit_order_sell(SYMBOL, price_entry, POS_SIZE) # открываем позицию в шорт
                            logger.info(f'Открыта позиция в ШОРТ⬇️⬇️⬇️ по цене {price_entry}, время: {date_entry}')
                            send_tg_message(f'{BOT_NAME} Открыта позиция в ШОРТ⬇️⬇️⬇️ по цене {price_entry}, время: {date_entry}')
                            ...
                            while open_position == True:
                                ...
                                if df_5m['close'].iloc[-1] < TP_1: # если цена дошла до TP_1
                                    ...
                                    # закрываем TP_1
                                    send_limit_order_buy(SYMBOL, price_tp_1, POS_SIZE/4)
                                    ...
                                    while open_position_3_4 == True:
                                        ...
                                        if df_5m['close'].iloc[-1]  < TP_2: # если цена дошла до TP_2 
                                            ...
                                            # закрываем TP_2
                                            send_limit_order_buy(SYMBOL, price_tp_2, POS_SIZE/4)
                                            qty_current -= POS_SIZE/ 4 # обновляем значение действующего объема
                                            ...
                                            while open_position_1_2 == True:
                                                ...
                                                if df_5m['close'].iloc[-1] < TP_3: # если цена дошла до TP_3 
                                                    ...
                                                    # закрываем TP_3
                                                    send_limit_order_buy(SYMBOL, price_tp_3, POS_SIZE/4)
                                                    qty_current -= POS_SIZE/ 4 # обновляем значение действующего объема
                                                    ...
                                                    while open_position_1_4 == True:
                                                        ...
                                                        if df_5m['close'].iloc[-1] < TP_4: # закрытие по TP_4
                                                            open_position_1_4 = False
                                                            price_tp_4 = df_5m['close'].iloc[-1]
                                                            date_stop = get_time() # получаем дату стопа
                                                            # закрываем позицию 
                                                            send_limit_order_buy(SYMBOL, price_tp_4, qty_current)
                                                            logger.info(f'Закрыта позиция по TP_4✅✅✅✅, время: {date_stop}, цена: {price_tp_4}')
                                                            send_tg_message(f'{BOT_NAME} Закрыта позиция по TP_4✅✅✅✅, время: {date_stop}, цена: {price_tp_4}')
                                                            time.sleep(PAUSE_BETWEEN_GET_DATA_IN_POSITION) # пауза
                                                            break
                                                            ...
                                                            # закрываем TP_4
                                                            send_limit_order_buy(SYMBOL, price_tp_4, POS_SIZE/4)                                                            
                                                            ...
                                                            date_stop = get_time() # получаем дату стопа
                                                            # закрываем позицию 
                                                            send_limit_order_buy(SYMBOL, price_stop, qty_current)
                                                            logger.info(f'Закрыта позиция по Б/У🟥, время: {date_stop}, цена: {price_stop}')
                                                            send_tg_message(f'{BOT_NAME} Закрыта позиция по Б/У🟥, время: {date_stop}, цена: {price_stop}')
                                                            time.sleep(PAUSE_BETWEEN_GET_DATA_IN_POSITION) # пауза
                                                            break
                                                            ...
                                                            # ищем TP_4
                                                            logger.info(f'Поиск точки TP_4, время: {get_time()}')
                                                    ...
                                                    # ищем TP_3
                                                    logger.info(f'Поиск точки TP_3, время: {get_time()}')
                                                    time.sleep(PAUSE_BETWEEN_GET_DATA_IN_POSITION)
                                            ...
                                            # закрываем позицию 
                                            send_limit_order_buy(SYMBOL, price_stop, qty_current)
                                            logger.info(f'Закрыта позиция по Б/У🟥, цена: {price_stop}, время: {date_stop}')
                                            send_tg_message(f'{BOT_NAME} Закрыта позиция по Б/У🟥,цена: {price_stop}, время: {date_stop}')
                                            time.sleep(PAUSE_BETWEEN_GET_DATA_IN_POSITION) # пауза
                                            break
                                            ...
                                            # ищем TP_2
                                            logger.info(f'Поиск точки TP_2, время: {get_time()}')
                                            time.sleep(PAUSE_BETWEEN_GET_DATA_IN_POSITION)    



                                elif df_5m['close'].iloc[-1] > stop_loss: # если ниже sma
                                    ...
                                    # ищем TP_1
                                    logger.info(f'Поиск точки TP_1, время: {get_time()}')
                                    time.sleep(PAUSE_BETWEEN_GET_DATA_IN_POSITION)

                        else:  
                            # контр-тренд, возврат в начало алгоритма
                ...
                # ищем силу тренда падающего

#http://127.0.0.1:5000/predict?shop_id=4886&user_id=1515915626151791662&limit=3&item_ids=21640,21641                   

@app.route('/')
def index():
    mess_1 = "The app SHORT_V3 is ready!!"
